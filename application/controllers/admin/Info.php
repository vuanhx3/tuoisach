...
class Info extends MY_Controller
{
	...
	function index()
	{
	    ...
	    $config['base_url']   = admin_url('info/index'); // link hien thi ra danh sach san pham 
		    ...
		    if($this->form_validation->run())
		    {
		       ...
		       // lưu dữ liệu cần thêm
		         $data = array(
	                ...
	                if ($this->info_model->create($data)){
	                    // tạo nội dung thông báo
	                    $this->session->set_flashdata('message', 'Thêm mới chuyên đề thành công !');
	                }else {
	                    $this->session->set_flashdata('message','Thêm chuyên đề không thành công !');
	                }
	...
	function check_slug_add()
	{
		...
		if($this->info_model->check_exists($where))
		{
	...
	{
		$slug = $this->input->post('slug');
	    $info = $this->info_model->get_info($this->uri->rsegment(3));

	     if($this->uri->rsegment('3')){
	       $conditional = $this->info_model->get_list(array('where' => array('slug !=' =>$info->slug,'slug'=>$slug)));
	      }
	     else{
	        $conditional = $this->info_model->get_list(array('where' => array('slug'=>$slug)));
	      }
   ...
   /*ham edit bai viet meo vat*/
	function edit()
	{
		...
		if(!$info)
		{
			$this->session->set_flashdata('message', 'Không tồn tại chuyên đề này');
			redirect(admin_url('info'));
		}
		$this->data['info'] = $info;
		    ...
		    if($this->form_validation->run())
		    {
		       ...
		       // lưu dữ liệu cần thêm
		         $data = array(
		            ...
		            // Thêm mới vào cơ sỡ dữ liệu
	                if ($this->info_model->update($id, $data)){
	                    // tạo nội dung thông báo
	                    $this->session->set_flashdata('message', 'Cập nhật chuyên đề thành công !');
	                }else {
	                    $this->session->set_flashdata('message','Cập nhật chuyên đề không thành công !');
	                }
	                redirect(admin_url('info'));
		    }
	    }
	...
	function delete()
	{
	  ...
	  $this->_del($id); 
	  // tạo nội dung thông báo
	  $this->session->set_flashdata('message','Xóa chuyên đề thành công !');
	  redirect(admin_url('info'));
	}
	...
	private function _del($id)
	{
	  ...
	  $news = $this->info_model->get_info($id);
	  if(!$news)
	  {
	...
	function viewcontent()
	{
		...
		if(!$info)
		{
			$this->session->set_flashdata('message', 'Chuyên đề không tồn tại!');
           redirect(admin_url('info'));
		}
		$this->data["info"] = $info;
		...
		// load sang view
		$this->data["temp"] = "admin/info/viewcontent";
